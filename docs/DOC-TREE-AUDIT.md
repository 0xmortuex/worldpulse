...
# Second audit — sources against coverage

The first audit checked documented claims against code. It could not have found this,
because nothing *claimed* it: **four sources are referenced by shipped app code and have
no fixture, no contract test and no recorded shape.** Three of them back panels that
shipped in steps 3, 4 and 7.

Generated by reading `data/sources.json`, `tests/fixtures/index.ts`, `tests/contracts.test.ts`
and grepping `src/` — not assembled by hand.

## Sources with shipped code depending on them

| Source | Panel | verifiedAgainst | Fixture | Contract test | Referenced in `src/` |
| --- | --- | --- | --- | --- | --- |
| `worldbank` | economy | **live** | yes | yes | yes (4) |
| `usgs-quakes` | layers | **live** | yes | yes | yes (3) |
| `wikidata-sparql` | government | documentation | yes | yes | yes (6) |
| `gdelt-doc` | news | documentation | yes | yes | yes (4) |
| `nasa-eonet` | layers | documentation | **NO** | **NO** | yes (2) — step 7 |
| `wikipedia-rest` | government | documentation | **NO** | **NO** | yes (2) — steps 3, 4 |
| `wikimedia-commons` | header | documentation | **NO** | **NO** | yes (1) — step 3 |
| `comtrade` | economy | documentation | **NO** | **NO** | yes (1) — gallery only |

## Sources with no shipped code yet

All 25 have no fixture and no contract test, which is appropriate — nothing depends on
them. Listed so the absence is recorded rather than assumed:

`abgeordnetenwatch`, `camara-br`, `congress-gov`, `eia`, `eu-sanctions`,
`exchangerate-host`, `fcdo-advisories`, `frankfurter`, `howtheyvote-data`,
`iptv-org-channels`, `iptv-org-streams`, `naturalearth`, `nosdeputes`, `ofac-sdn`,
`openparliament-ca`, `opensanctions`, `riksdagen`, `smartraveller`,
`state-dept-advisories`, `theyvoteforyou`, `tweedekamer`, `ucdp-candidate`, `ucdp-ged`,
`uk-commons-votes`, `worldbank-milex`
...
| Referenced in `src/` | 8 |
| With a fixture | 4 |
| With a contract test | 4 |
| **Shipped code, no contract test** | **4** |

`comtrade` is the mildest of the four: its only reference is a gallery entry rendering
the *unconfigured* state, so nothing parses a Comtrade response yet. The other three
parse real responses in shipped panels.

**Limitation of this grid, stated rather than hidden:** "referenced in `src/`" matches the
source *id string*. `naturalearth` scores "no" because the app reaches it through the
`world-atlas` package rather than by id — so the grid understates dependency for any
source consumed without naming its registry id. It is a lower bound on coverage debt, not
an upper one.
...
| # | Finding | Closed by |
| --- | --- | --- |
| 6 | A source could be flipped to `live` with no fixture and no contract test | `check-deploy-gate.mjs` now blocks it; observed firing on a planted `nasa-eonet` |
| 7 | The gate printed `[ ok ]` beside a source that had just recorded a problem | mark now reflects the outcome, not the claimed status |
| 8 | `loadSample`'s docstring claimed "identical assertions … only the input is swapped" — untrue, it fetched `probeUrl` rather than the fixture's `requestUrl` | fixed and now true; guarded by `tests/fixtures.test.ts` |
