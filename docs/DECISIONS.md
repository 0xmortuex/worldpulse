...
| 11b | **The tone timeline is removed, not approximated. Done 2026-08-12.** Three reasons, all of which had to hold: **(1)** its only source, GDELT, is unreachable — **eight attempts, eight failures**, zero successful responses ever observed; **(2)** **no fallback source exists** — the 15 curated RSS feeds carry headlines, outlets and timestamps, and none publishes a sentiment score, so there is nothing to fall back *to*; **(3)** **computing sentiment ourselves was ruled out** — that would be this app authoring an editorial judgement about coverage it cannot cite, which is G1's rule applied to news. N2's caveat existed precisely because the number was a machine estimate rather than a measurement; an estimate we produced ourselves would have no source at all, and would be the one number on the surface with no provenance to inspect. A panel that cannot be sourced is removed rather than degraded, because a degraded sentiment chart still renders a number and readers do not read caveats as hard as they read charts. |
| 11d | **What the removal took with it, recorded so its absence is deliberate rather than lost.** `src/news/tone-chart.ts` and `src/sources/gdelt-tone.ts` deleted; three tone fixtures deleted; six unit tests and seven browser assertions removed; eight CSS rules removed. The step-6 mutation was **re-pointed rather than dropped** — V5 requires one mutation per step, and deleting the only step-6 mutation would have left the news tab unmutated while still appearing covered. The article-grouping and text-direction helpers that shared `gdelt-tone.ts` were **kept** and moved to `src/sources/news-text.ts`: they implement N4, N6 and rule 9, are not about sentiment, and apply to the RSS feeds that replace GDELT. |
...
| S4a | **S4 was written when a verify cost ~18 minutes and a full mutation suite ~65. Under the GPU they cost 103 seconds and ~21 minutes.** A full mutation suite before every push is now affordable where it was not, so that half of S4 is a floor rather than a ceiling: run it. What stands is the reason — a check nobody can afford to run stops being run — and that reason now bites at a different threshold. Re-read S4 whenever the harness gets materially faster or slower; the discipline follows the cost, not the other way round. |
...
| S7 | **When a decision amends a criterion a live goal depends on, the goal text is amended in the same breath.** S6's sibling. A goal whose condition can no longer be met does not fail quietly — it burns its evaluator's turns proving the impossibility, over and over, because each evaluation is correct and the conjunction is still unsatisfiable. Measured: the Fact-model migration required "both P3 fixtures shown rendering caveated"; question 14 struck one fixture and amended the other to the reachable case; the goal text was not amended with it, and the condition was re-evaluated as unmet for the rest of the session while the work it gated was finished and verified. **The amendment record is the decision that struck the criterion** — here question 14 — and the goal is closed administratively against it. Corollary for writing goals: state conditions against DOCUMENTS that can absorb an amendment (a per-source gate table, the verification battery, `OPEN-QUESTIONS.md`) rather than against named artefacts a future decision might strike. |
